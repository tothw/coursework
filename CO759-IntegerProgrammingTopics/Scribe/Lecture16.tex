...
\begin{proof}
\paragraph{(1)}
We proceed by induction on the dimensions of $A$. In the base case, $A$ a scalar, the result is trivial. In the inductive case we will aim to show a sequence of the allowed operations will transform $A$ into a matrix of the form:
\begin{align*}
\begin{bmatrix}
\alpha & 0\\
x & \bar{A}
