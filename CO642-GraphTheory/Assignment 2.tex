...
\end{align*}
\paragraph{}
Let $v \in V(G)$. Let $ch_F(v)$ denote the final charge of $v$. The degree of $v$ is at least six. If $d(v) = 6$ then either $v$ is incident with less than six facial triangles or $v$ has a neighbour of degree greater than six. Suppose $v$ is incident with less than six facial triangles. Then $v$ is incident with at least one face of length at least four. By rule $(2)$ this face gives $\frac{6}{c}$ charge to $v$. Notice $v$ gives away no charge. Thus $ch_F(v) \geq ch(v) + \frac{6}{c} = 6 - 6 - \frac{6}{|V|}g + \frac{6}{c} > -\frac{6}{cg}g + \frac{6}{c} = 0$. 
\paragraph{}
If $d(v) 
