...
\section*{1}
\paragraph{}
Let $G$ be a graph with $n$ vertices and minimum degree $\delta > 1$. Let $X \subseteq V(G)$ chosen at random such that for any $v \in V(G)$, $Pr(v \in X) = \frac{ln(\delta)}{\delta + 1}$. Let $p = \frac{ln(\delta)}{\delta + 1}$. Then $Pr(|X| = k ) = {n \choose k} p^k (1-p)^k$, and $E[|X|] = np$. Let $u  \adjacent v$ denote that $u$ is adjacent to $v$ for any $u,v \in V(G)$. Since $G$ has minimum degree $\delta$, for any $u, v \in V(G)$, $Pr(u \adjacent v) \geq \frac{\delta}{n}$. Since $G$ has $n$ vertices, $Pr(u = v) = \frac{1}{n}$. Now $Pr(u \notadjacent v \text{ and } u \neq v) = 1 - (Pr(u \adjacent v) + Pr(u = v)) = \frac{n - 1- \delta}{n}$.
\paragraph{}
