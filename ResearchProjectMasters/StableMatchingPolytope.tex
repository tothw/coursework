...
Since $w = n^*(v)$, $x(\delta^{>w}(v)) = \emptyset$ (also notice $\chi(\delta^{>w}(v)) = 0$). Therefore in fact we have that
$$x(\delta^{\geq v}(w)) = 1.$$
...
\end{align*}
Since $V_0, V_1 \subseteq W$ this contradicts Lemma $2$ part $3$. Therefore there exists $v \in V \backslash W$. Let $w = n^*(v)$. Let $e = vw \in E$. By Lemma $3$, $e \not \in T$. Since $v \not\in W$, $e$ keeps at least $\frac{1}{2}$ token. Therefore $|E| > |W| + |T|$ contradicting condition $4$ of Lemma $2$. 
\end{proof}
