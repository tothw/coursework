...
\section*{20}
\paragraph{}
Let $C$ be closed and convex and let $u \not\in C$. Let $x$ lie on the half line from $\pi_C(u)$ through $u$. That is there exists $\alpha \geq 0$ for which $$ x = (1-\alpha)\pi_C(u) + \alpha u.$$
We consider two cases: either $\alpha \geq 1$ or $\alpha < 1$.
\paragraph{}
First suppose that $\alpha \geq 1$. By definition $$\pi_C(x) = \arg\min_{y\in C} ||y-x||.$$
Since $x = (1-\alpha)\pi_C(u) + \alpha u$ we have
$$\pi_C(x) = \arg\min_{y\in C} || (\alpha -1)\pi_C(u) + y - \alpha u ||.$$
If we let $$S = \{\frac{\alpha-1}{\alpha}\pi_C(u) + \frac{1}{\alpha}y : y \in C\}$$
then $$\pi_C(x) = (1-\alpha)\pi_C(u) + \alpha \cdot(\arg\min_{z\in S} |\alpha|\cdot ||z-u||).$$
Now we claim that $\pi_C(u) \in S$ and $S \subseteq C$. If the claim holds then $$\arg\min_{z\in S} |\alpha|\cdot||z-u|| = \pi_C(u)$$
and hence $\pi_C(x) = \pi_C(u)$ as desired. To see the claim first observe that if $y = \pi_C(u)$ then
$$\frac{\alpha-1}{\alpha}\pi_C(u) + \frac{1}{\alpha}y = \pi_C(u)$$
and so $\pi_C(u) \in C$. To see that $S\subseteq C$ we use the fact that $C$ is convex and observe that every point in $S$ is a convex combination of points in $C$. Indeed, since $$\frac{\alpha-1}{\alpha} \geq 0$$ as $\alpha \geq 1$, and $$\frac{1}{\alpha} \geq 0$$ as $\alpha \geq 0$,
and $$\frac{\alpha - 1}\alpha + \frac{1}{\alpha} = 1$$
we have that every point in $S$ is a convex combination of points in $C$. Since $C$ is convex, $C$ is closed under convex combinations and hence contains $S$. Therefore the claim holds and we are done in this case.
\paragraph{}
Now suppose that $\alpha < 1$. This case is very similar to the previous case but we interchange the roles of $x$ and $u$. By definition
$$\pi_c(u) = \arg\min_{y\in C} ||y-u||.$$
Since $u = \frac{1}{\alpha}x + \frac{\alpha -1}{\alpha}\pi_C(u)$ we have $$\pi_C(u) = \arg\min_{y\in C} ||\frac{1-\alpha}{\alpha}\pi_C(u) + y - \frac{1}{\alpha}x ||.$$
If we let $$S = \{(1-\alpha)\pi_C(u) + \alpha y : y \in C\}$$ then $$\pi_C(u) = \frac{\alpha-1}{\alpha} \pi_C(u) + \frac{1}{\alpha}(\arg\min_{z\in S} |\frac{1}{\alpha}|\cdot||z - x||)$$
Similarly
\end{document}
