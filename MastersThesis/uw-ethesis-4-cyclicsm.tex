...
\end{proof}
\begin{corollary}
Let $G=(A\cup B \cup C, E)$ be the hypergraph for an instance of $C3GSM$ with $n$ as in lemma \ref{lemma:partialstab}. If one of $A,B,C$, say without loss $A$, is such that for all $a,a' \in A$, $f(G,a,1) = f(G,a',1)$ then $G$ has a stable matching.
\end{corollary}
\begin{proof}
Let $a_1, \dots, a_n \in A$. Suppose that $b = f(G,a_1,1) =  \dots = f(G,a_n,1)$. Let $c = f(G,b,1)$. Then there exists some $a_k$ such that $a_k = f(G,c,1)$. Since $a_k \in A$ we may invoke corollary \ref{cor:1cycle} with $(a_k, b, c)$ to complete the proof.
\end{proof}
\begin{lemma} Let $G=(A\cup B \cup C, E)$ be the hypergraph underlying an instance of $C3GSM$ of size $n$ where $n-1$ is the best known size satisfying conjecture \ref{conj:stab}. Let $M$ be a matching in $G$. Suppose that for all $b\in B\cap V(M)$ and $c\in C\cap V(M)$, $b,c \not\in V(\cB(G,M))$. Also suppose that for all $a \in A\cap V(M)$ if $abc \in \cB(G,M)$ then $b \not\in V(M)$. If $b = f(G,a,1)$ for all $a \in A\backslash V(M)$ then $G$ has a stable matching.
\end{lemma}
\begin{proof}
